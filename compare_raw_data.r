...
######## compare mainly Quarterly and monthly data
	...
	if(nrow(test)>0){
		...
		for (i in 1:nrow(test)){
			...
			X <- readRDS(paste0(ilo:::path$sys, 'ILO_Data/STI/', test$value[i])) %>% select_(.dots = c("collection", "ref_area", "source", "indicator", "sex", "sex_version", "classif1", "classif1_version", "classif2", "classif2_version", "time", "obs_value", "obs_status", "note_classif", "note_indicator", "note_source"))
			
			for (j in 1:length(ref_collection)){
				X1 <- X %>% filter(collection %in% ref_collection[j])
				if(nrow(X1) > 0){			
					X1 %>%
						data.table:::fwrite(file = paste0(ilo:::path$sys, 'ILO_Data/_by_ref_area_',ref_collection,'/NEW_', test$value[i] %>% str_replace('.rds', '.csv')), na = '')
				}
				rm(X1)
			}
			rm(X)
	...
	for(i in 1:nrow(test)){
		...
		STI <- readRDS(paste0(ilo:::path$sys, 'ILO_Data/STI/', test$value[i])) %>%  mutate_all(as.character) %>%  select_(.dots = c("collection", "ref_area", "source", "indicator", "sex", "sex_version", "classif1", "classif1_version", "classif2", "classif2_version", "time", "obs_value", "obs_status", "note_classif", "note_indicator", "note_source", "freq_code"))
	...
	for (i in 1:length(ref_col)){
		...
		for (j in 1:length(ref_type)){
			...
			for(k in 1:nrow(ref_file)){
				...
				if(nrow(X) > 300000){
					X %>% data.table:::fwrite(file = paste0(ilo:::path$sys, 'ILO_Data/ZZZ_ilostat/',ref_type[j],'_',ref_col[i], '_', count,'_', Sys.Date(), '.csv'), na = '')
					X <- NULL
					count <- count + 1
				}
...
compare_raw_data_ANNUAL <- function(ref_collection = 'YI', ref_ref_area = NULL, ref_freq_code  = 'A'){
	...
	if(nrow(test)>0){
		...
		for (i in 1:nrow(test)){
			...
			X <- readRDS(paste0(ilo:::path$sys, 'ILO_Data/STI/', test$value[i])) %>% select_(.dots = c("collection", "ref_area", "source", "indicator", "sex", "sex_version", "classif1", "classif1_version", "classif2", "classif2_version", "time", "obs_value", "obs_status", "note_classif", "note_indicator", "note_source")) %>% mutate_all(as.character)
