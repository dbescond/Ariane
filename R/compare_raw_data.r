...
compare_raw_data <- function(ref_collection = 'STI', ref_area = NULL, freq_code  = c('A', 'Q','M')){
######## compare mainly Quarterly and monthly data
	...
	ref_file_ORA <- list.files(paste0(ilo:::path$sys, 'ILO_Data/ORA/')) %>% as_data_frame %>% filter(str_sub(value, 5,5) %in% freq_code)
	ref_file_STI <- list.files(paste0(ilo:::path$sys, 'ILO_Data/STI/')) %>% as_data_frame %>% filter(str_sub(value, 5,5) %in% freq_code)
	...
	for(i in 1:nrow(test)){
		...
		ORA <- readRDS(paste0(ilo:::path$sys, 'ILO_Data/ORA/', test$value[i])) %>% filter(collection %in% ref_collection) %>% mutate_all(as.character)
		STI <- readRDS(paste0(ilo:::path$sys, 'ILO_Data/STI/', test$value[i])) %>% filter(collection %in% ref_collection) %>% mutate_all(as.character)
...
merge_raw_data_ANNUAL <- function(ref_collection = 'YI'){
	...
	ref_file_STI_Upload_By_Country <- list.files(paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/')) %>% as_data_frame %>%  
		separate(value, c('collection', 'type', 'ref_area', 'freq'), sep = '_', remove = FALSE) %>% mutate(freq = str_sub(freq, 1,1))
		...
		for (j in 1:length(ref_type)){
			...
			for(k in 1:nrow(ref_file)){
				...
				if(ref_type[j] %in% 'DEL'){
					...
					X <- bind_rows(X, 
							read_csv(
									paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/',ref_file$value[k]), 
									col_types = cols(
												collection = col_character(),
												ref_area = col_character(),
												source = col_character(),
...
#' @export	
compare_raw_data_ANNUAL <- function(ref_collection = c('YI'), ref_area = NULL, freq_code  = c('A')){
	...
	ref_file_STI_Upload_By_Country <- list.files(paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/')) %>% as_data_frame
	for(i in 1:nrow(ref_file_STI_Upload_By_Country)){
		unlink(paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/', ref_file_STI_Upload_By_Country$value[i]))
	}
	rm(ref_file_STI_Upload_By_Country)
		...
		for (i in 1:nrow(test)){
			...
			if(nrow(X)>0){ 
				for (j in 1:length(ref_collection)){
					...
					if(nrow(X1) > 0){			
						X1 %>% distinct(collection, ref_area, source, indicator, sex_version, classif1_version, classif2_version, time) %>% 
							data.table:::fwrite(file = paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/',ref_collection[j],'_DEL_', test$value[i] %>% str_replace('.rds', '.csv')), na = '')
					}
					rm(X1)
				}
			rm(X)
	...
	if(nrow(test)>0){
		...
		for (i in 1:nrow(test)){
			...
			X <- readRDS(paste0(ilo:::path$sys, 'ILO_Data/STI/', test$value[i])) %>% select(-freq_code) %>% mutate_all(as.character)
			...
			if(nrow(X) > 0){			
				X %>%
					data.table:::fwrite(file = paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/',unique(X$collection),'_NEW_', test$value[i] %>% str_replace('.rds', '.csv')), na = '')
			}
...
compare_annual <- function(test, i){
...
TEST_IND <- Ariane:::CODE_ORA$T_CIC_COL %>% filter(CIC_COLLECTION_CODE %in% c('YI')) %>% select(indicator = CIC_INDICATOR_CODE) %>% .$indicator
		...
		############## step 0 manage col data on ORA
			...
			#
				...
				NEW <- X1 %>% left_join(check_STI, by = c("collection", "ref_area", "source", "indicator", "time")) %>% 
					mutate(TEST = ifelse( ss.x == ss.y & n.x > n.y, "equivalent_more_points_on_YI", NA)) %>% 
					mutate(TEST = ifelse( ss.x == ss.y & n.x < n.y, "DEL_equivalent_more_points_on_STI", TEST)) %>% 
					mutate(TEST = ifelse( ss.x == ss.y & n.x == n.y, "DEL_equivalent_same_points_on_STI", TEST)) %>% 
					mutate(TEST = ifelse( ss.x < ss.y & n.x >= n.y, "better_version_on_YI_more_points_on_YI", TEST)) %>% 
					mutate(TEST = ifelse( ss.x < ss.y & n.x < n.y, "DEL_better_version_on_YI_more_points_on_STI", TEST)) %>% 
					mutate(TEST = ifelse( ss.x > ss.y & n.x == n.y, "DEL_better_version_on_STI_same_points_on_STI", TEST)) %>% 
					mutate(TEST = ifelse( ss.x > ss.y & n.x < n.y, "DEL_better_version_on_STI_more_points_on_STI", TEST)) %>% 
					mutate(TEST = ifelse( ss.x > ss.y & n.x > n.y, "better_version_on_STI_more_points_on_YI", TEST)) %>% 
...
############### new starting point for a normal comparison
		...
		if( nrow(ON_STI) == 0 & nrow(ON_ORA) > 0){
			...
			if(nrow(check_STI)>0){
					...
					if(nrow(X1) > 0){			
						X1 %>% mutate_all(as.character) %>%
							data.table:::fwrite(file = paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/',ref_collection,'_DEL_', test$value[i] %>% str_replace('.rds', '.csv')), na = '')
					}
					rm(X1)
			}
		...
		if(nrow(ON_STI) > 0 & nrow(ON_ORA)  == 0){
				...
				if(nrow(X1_rev) > 0){			
					X1_rev %>%  mutate_all(as.character) %>% mutate(obs_value = as.numeric(obs_value)) %>%  select(-contains('_version')) %>%  select(-contains('TEST')) %>%
						data.table:::fwrite(file = paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/',ref_collection,'_REV_', test$value[i] %>% str_replace('.rds', '.csv')), na = '')
				}
				rm(X1_rev)
				X1 <- X1 %>% filter(check %in% 0) %>% select(-check)
				...
				if(nrow(X1) > 0){			
					X1 %>%  mutate_all(as.character) %>% mutate(obs_value = as.numeric(obs_value)) %>% select(-contains('_version')) %>% select(-contains('TEST')) %>%
						data.table:::fwrite(file = paste0(ilo:::path$sys, 'ILO_Data/ILOSTAT_BY_REF_AREA/',ref_collection,'_NEW_', test$value[i] %>% str_replace('.rds', '.csv')), na = '')
				}
				rm(X1)
