...
#'  head(res)					
#' @export
getDataOracle_NEW2 <- function(data_query, extra = FALSE, Save  = FALSE, mv = FALSE, con ){
# data_query <- "where collection_code in ('STI') and Country_Code in ('ABW') and  time_freq in ('A')"; extra = TRUE; Save  = TRUE; mv = TRUE
#require(rJava)
#require(RJDBC)
...
X <- X %>% 
	...
	invisible(gc(reset = TRUE))
	invisible(gc(reset = TRUE))
	test_frq <- unique(X$freq_code)
	test_cou <- unique(X$ref_area)

	test_col <- unique(X$collection)
	saveRDS(X, file = paste0(ilo:::path$sys, 'ILO_Data/ORA_NEW/',test_col, '_',test_cou, '_', test_frq, ".rds"))

	rm(X, test_frq,test_cou, test_col )
	invisible(gc(reset = TRUE))
	invisible(gc(reset = TRUE))
	invisible(gc(reset = TRUE))
	invisible(gc(reset = TRUE))
...
#' @export
getDataOracle_NEW <- function(data_query, extra = FALSE, Save  = FALSE, mv = FALSE, con ){
# data_query <- "where Country_Code in ('AND') and TIME_FREQ in ('A')"; extra = TRUE; Save  = TRUE; mv = TRUE
#require(rJava)
#require(RJDBC)
...
if(Save & !length(test[test %in% 'A'] %in% 'A') == 0) {
		saveRDS(X %>% filter(freq_code %in% 'A') ,file = paste0(ilo:::path$sys, 'ILO_Data/ORA/',as.character(unique(X$ref_area)),"_A.rds"))
}	
if(Save & !length(test[test %in% 'Q'] %in% 'Q') == 0) {
		saveRDS(X %>% filter(freq_code %in% 'Q') ,file = paste0(ilo:::path$sys, 'ILO_Data/ORA/',as.character(unique(X$ref_area)),"_Q.rds"))
}	
if(Save & !length(test[test %in% 'M'] %in% 'M') == 0) {
		saveRDS(X %>% filter(freq_code %in% 'M') ,file = paste0(ilo:::path$sys, 'ILO_Data/ORA/',as.character(unique(X$ref_area)),"_M.rds"))
}	
if(Save){
	rm(X)
	invisible(gc(reset = TRUE))
...
# data_query <- "where Country_Code in ('ASM')"; extra = TRUE; Save  = TRUE; mv = TRUE
#require(rJava)
#require(RJDBC)
